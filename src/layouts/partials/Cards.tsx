...
const Card = ({i, service, progress, range, targetScale}:
    {i:number, service:Service, progress:any, range:any, targetScale:any}) => {
          ...
          <div ref={container} className={`${styles.cardContainer}`} >
            ...
            >
              <h2 className={`${i% 2 === 0 ? "text-[#FBDC6D]":"text-[#1b1e49]"} `}
               dangerouslySetInnerHTML={markdownify(service.heading)}/>
              <div className={styles.body}>
                <div className={styles.description}>
                  <p className={`desc ${i% 2 === 0 ? "text-[#FBDC6D]":"text-[#1b1e49]"} text-justify`} dangerouslySetInnerHTML={markdownify(service.description)}/>
                  ...
                  <span className={`spsp flex items-center gap-x-3 mt-12 ${i% 2 === 0 ? "bg-[#FBDC6D]":"bg-[#1b1e49]"} rounded-full w-fit px-4 py-2 hover:shadow-xl hover:scale-105 transition-all duration-500`}>
                    <Link className={`linkk ${i% 2 === 0 ? "text-[#1b1e49]":"text-[#FBDC6D]"}`} href={service.serviceLink} target='_blank' >See service details</Link>
