...
const Card = ({i, Heading, ServiceImg, Description, progress, range, targetScale}:
    {i:number, Heading:string, ServiceImg:string, Description:string, progress:any, range:any, targetScale:number}) => {
        ...
        return (
          <div ref={container} className={`${styles.cardContainer}`} >
            ...
            >
              <h2 className={`text-[28px] ${i% 2 === 0 ? "text-[#FBDC6D]":"text-[#1b1e49]"}`}
               dangerouslySetInnerHTML={markdownify(Heading)}/>
              <div className={styles.body}>
                <div className={styles.description}>
                  <p className={`desc ${i% 2 === 0 ? "text-[#FBDC6D]":"text-[#1b1e49]"}`} dangerouslySetInnerHTML={markdownify(Description)}/>
                  ...
                  <span className={`spsp flex items-center gap-x-3 mt-12 ${i% 2 === 0 ? "bg-[#FBDC6D]":"bg-[#1b1e49]"} rounded-full w-fit px-4 py-2 hover:shadow-xl hover:scale-105 transition-all duration-500`}>
                    <Link className={`linkk ${i% 2 === 0 ? "text-[#1b1e49]":"text-[#FBDC6D]"}`} href={"#"} target="_blank">Read more</Link>
